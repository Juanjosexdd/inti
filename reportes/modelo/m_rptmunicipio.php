<?php 
	...
	class Repmunicipio extends Conectar
	{
		...
		{ // consultar por aproximidad	
			$conectar = parent::conexion();
			 ...
			 {
				 $sql ="
				 		SELECT m.codmunicipio, m.nombre nommun, e.codestado, e.nombre nomest, p.codpais, p.nombre nompais, m.estatus
			 			FROM municipio m 
			 			INNER JOIN estado e ON m.codestado = e.codestado
			 			INNER JOIN pais p ON e.codpais = p.codpais
			 			WHERE m.estatus = $estatus 
			 				and m.nombre like UPPER(('%$dato%'))  
			 			ORDER BY p.nombre, e.nombre, m.nombre";
			...
			else
			{
				$conectar = parent::conexion();
			 			...
			 			INNER JOIN estado e ON m.codestado = e.codestado
			 			INNER JOIN pais p ON e.codpais = p.codpais
			 			WHERE m.nombre like UPPER(('%$dato%'))  
			 			ORDER BY p.nombre, e.nombre, m.nombre";
							$sql = $conectar->prepare($sql);
							$sql->execute();
							return $sql->fetchAll();
			...
			{
				$conectar = parent::conexion();
					$sql =" SELECT m.codmunicipio, m.nombre nommun, e.codestado, e.nombre nomest, p.codpais, p.nombre nompais, m.estatus
			 			FROM municipio m 
			 			INNER JOIN estado e ON m.codestado = e.codestado
			 			INNER JOIN pais p ON e.codpais = p.codpais
			 			ORDER BY p.nombre, e.nombre, m.nombre";
		...
		function reporte($result = false){
			...
			$obj_pdf->Cell(65, $celdaLargo, utf8_decode('Municipio'),0,0,'C'); // celda titulo tabla
			...
			foreach ($result as $key => $row) 
			{
				...
				$obj_pdf->SetWidths(array(65,50,45,35));
			...
			$obj_pdf->Output('listado_municipio.pdf', 'I'); // salida del pdf.	
		}
	}
?>