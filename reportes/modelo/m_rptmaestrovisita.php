<?php 
	...
	class Repvisita extends Conectar
	{
			...
			{ // consultar por aproximidad
				$conectar = parent::conexion();
				 if($estatus != "")
				 {
					 $sql ="
					 		SELECT *
				 			FROM maestrovisita v
				 			WHERE v.estatus = $estatus 
				 				and (v.nombre like UPPER(('%$dato%')) OR v.descripcion like UPPER(('%$dato%')))
				 			ORDER BY v.nombre, v.descripcion";
				...
				else
				{
					$conectar = parent::conexion();
								$sql ="
										SELECT *
							 			FROM maestrovisita v
							 			WHERE (v.nombre like UPPER(('%$dato%')) OR v.descripcion like UPPER(('%$dato%')))
							 			ORDER BY v.nombre, v.descripcion";
								$sql = $conectar->prepare($sql);
								$sql->execute();
								return $sql->fetchAll();
		...
		function reporte($result = false)
		{
			...
			$obj_pdf->Output('listado_tipo_visita.pdf', 'I'); // salida del pdf.	
		}
	}
?>