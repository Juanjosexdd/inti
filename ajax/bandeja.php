  ...
  _POST(); 
    switch( $_GET["op"] ){
      case 'ProcesarTramite': 
          ...
          $data_estatus = $tramite->get_orden_estatus_dpto($estatus); 

          //print_r($data_estatus);
      ...
      case 'agregarRecaudos': 
        ...
        if(!empty($idtramite) && !empty($idrecaudo) && !empty($codtramite)){
          ...
          if(is_array($datos)==true && count($datos)==0){
            ...
            $where = "WHERE r.estatus = 1 AND mtr.codtramite = '{$codtramite}'";
      ...
      case 'cargarRecaudos': 
          ...
          $where = "WHERE r.estatus = 1 AND mtr.codtramite = '{$_POST['codtramite']}'";
        ...
        if( $datos ){
            ...
            foreach($datos as $row){
              ...
              if( $row["codestatus"] == -1 || $row["codestatus"] == 5){ // Anular trámite
                ...
                $accion .= '
                <button type="button" onClick="anularTramite('.$row["idtramite"].','.$row["codtramite"].',\''.$datociudadano.'\',\''.$row["tramite"].'\',\''.$date.'\',\''.$row["sector"].'\',\''.$row["estatus"].'\',\''.$row["observacion"].'\');" data-toggle="modal" data-target="#tramiteAnularModal" title="Anular" id="'.$row["idtramite"].'" class="btn btn-danger btn-md update"><i class="glyphicon glyphicon-edit"></i><i class="fas fa-trash-alt"></i></button>';
              }

              if( $row["codestatus"] == 0 || $row["codestatus"] == 1){ // Agregar Recaudos

                if( $row["codestatus"] == 1 ){
                  if( $_SESSION['coddpto'] != '1' ){
                    $accion .= '
                <button type="button" disabled title="Agregar Recaudos" class="btn btn-success btn-md update"><i class="glyphicon glyphicon-edit"></i><i class="fas fa-plus"></i></button>';
                    ...
                    $accion .= '
                <button type="button" onClick="listarRecaudos('.$row["idtramite"].','.$row["codtramite"].',\''.$datociudadano.'\',\''.$row["tramite"].'\',\''.$date.'\',\''.$row["sector"].'\',\''.$row["estatus"].'\',\''.$row["observacion"].'\');" data-toggle="modal" data-target="#tramiteRecaudoModal" title="Agregar Recaudos" id="'.$row["idtramite"].'" class="btn btn-success btn-md update"><i class="glyphicon glyphicon-edit"></i><i class="fas fa-plus"></i></button>';
              ...
              if( $row["codestatus"] == -1 || $row["codestatus"] == 5 || $row["codestatus"] == 0){ // Atender Solicitud 
                ...
                if( $row["codestatus"] == 1 ){
                  if( $_SESSION['coddpto'] != '2' ){
                    $accion .= '
                      <button type="button" disabled onclick="" class="btn btn-primary btn-md update"><i class="fa fa-check"></i></button>
                      ';
                  }else{
                    $accion .= '
                    <button type="button" onClick="procesarTramite('.$row["idtramite"].','.$row["codtramite"].',\''.$datociudadano.'\',\''.$row["tramite"].'\',\''.$date.'\',\''.$row["sector"].'\',\''.$row["codestatus"].'\',\''.$row["observacion"].'\');" data-toggle="modal" data-target="#tramiteProcesarModal" class="btn btn-primary btn-md update"><i class="fa fa-check"></i></button>
                    ';
                  }
