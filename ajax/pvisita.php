  ...
  switch($_GET["op"]){
    ...
    case "listar":
      ...
      foreach($datos as $row){
        ...
        if(empty($fechafin)){
          ...
          $sub_array[] = '<button type="button" onClick="finvisita('.$row["idvisita"].');"  id="'.$row["idvisita"].'" class="btn btn-danger btn-sm update"> <i class="fas fa-door-open"></i> </button>';
        } else {
          $sub_array[] = '<button type="button" onClick="finvisita('.$row["idvisita"].');"  id="'.$row["idvisita"].'" class="btn btn-danger btn-sm update disabled"> <i class="fas fa-door-open"></i> </button>';
        }
    ...
    case "finvisita":
      ...
      if(is_array($datos)==true && count($datos)==1){
        ...
        $visita->fin_visita($_POST['idvisita']);
      } 
