	...
	function mostrar(id){
		$.post("../ajax/mrif.php?op=mostrar",{id : id}, function(data, status){
	...
	function guardaryeditar(e){
		e.preventDefault();
		var formData = new FormData($("#rif_form")[0]);

		$.ajax({
			url: "../ajax/mrif.php?op=guardaryeditar",
		    type: "POST",
		    data: formData,
		    contentType: false,
		    processData: false,
    ...
    function cambiarEstado(id, est){
 		bootbox.confirm("¿Está seguro de cambiar de estatus?", function(result){
			if(result){
				$.ajax({
					url:"../ajax/mrif.php?op=activarydesactivar",
					method:"POST",
					data:{id:id, est:est},
