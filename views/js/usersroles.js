	...
	function init(){
		...
		$("#add_button").click(function(){
			...
			$(".modal-title").text("Registrar rol a usuario");
		});
	}
	...
	function listar(){
		tabla=$('#usersroles_data').dataTable({
			...
			"ajax":
					{
						url: '../ajax/usersroles.php?op=listar',
						type : "get",
						dataType : "json",						
						error: function(e){
	...
	function mostrar(id){
		$.post("../ajax/usersroles.php?op=mostrar",{id : id}, function(data, status){
	...
	function guardaryeditar(e){
		e.preventDefault();
		var formData = new FormData($("#usersroles_form")[0]);

		$.ajax({
			url: "../ajax/usersroles.php?op=guardaryeditar",
		    type: "POST",
		    data: formData,
		    contentType: false,
		    processData: false,
    ...
    function cambiarEstado(id, est){
 		bootbox.confirm("¿Está seguro de cambiar de estatus?", function(result){
			if(result){
				$.ajax({
					url:"../ajax/usersroles.php?op=activarydesactivar",
					method:"POST",
					data:{id:id, est:est},
